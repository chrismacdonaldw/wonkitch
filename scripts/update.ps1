...
function Find-WonkitchExecutable {
    $candidates = @(
        ...
        (Join-Path $env:ProgramFiles "wonkitch\wonkitch.exe")
    )
...
$installedExecutable = Find-WonkitchExecutable
$installedVersion = [version]"0.0.0"
if ($installedExecutable) {
    $installedVersion = ConvertTo-Version (Get-Item -LiteralPath $installedExecutable).VersionInfo.ProductVersion
}

if (-not $Force -and $installedVersion -ge $latestVersion) {
    Write-Host "wonkitch $installedVersion is already current ($($release.tag_name))."
    if (-not $NoLaunch -and $installedExecutable) {
...
if (-not $installedExecutable) {
    Write-Host "No installed wonkitch executable was found; this update will perform an installation."
}
...
try {
    ...
    Write-Host "Updated wonkitch from $installedVersion to $latestVersion."
    if (-not $NoLaunch -and $installedExecutable) {
        Start-Process -FilePath $installedExecutable
    }
} finally {
